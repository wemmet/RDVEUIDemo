...
@interface RDGPUImageFramebuffer()
{
    GLuint framebuffer;
...
void RDDataProviderReleaseCallback (void *info, const void *data, size_t size);
void RDDataProviderUnlockCallback (void *info, const void *data, size_t size);

@implementation RDGPUImageFramebuffer
...
- (id)initWithSize:(CGSize)framebufferSize textureOptions:(GPUTextureOptions)fboTextureOptions onlyTexture:(BOOL)onlyGenerateTexture;
{
    ...
    else if (!CGSizeEqualToSize(_size, CGSizeZero))//20180705 fix bug:添加MV后导出取消时崩溃
    {
...
- (void)generateFramebuffer;
{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            CVOpenGLESTextureCacheRef coreVideoTextureCache = [[RDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache];
            ...
            if (err)
            {
                if(0 == _size.height && _size.width == 0){
                    NSLog(@"rdFBO size: %f, %f", _size.width, _size.height);
...
- (void)destroyFramebuffer;
{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([RDGPUImageContext supportsFastTextureUpload] && (!_missingFramebuffer))
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            if (renderTarget)
            {
...
//    NSAssert(framebufferReferenceCount > 0, @"Tried to overrelease a framebuffer, did you forget to call -useNextFrameForImageCapture before using -imageFromCurrentFramebuffer?");
    ...
    {
        [[RDGPUImageContext sharedFramebufferCache] returnFramebufferToCache:self];
    }
}
...
void RDDataProviderUnlockCallback (void *info, const void *data, size_t size)
{
    RDGPUImageFramebuffer *framebuffer = (__bridge_transfer RDGPUImageFramebuffer*)info;
    
    [framebuffer restoreRenderTarget];
    [framebuffer unlock];
    [[RDGPUImageContext sharedFramebufferCache] removeFramebufferFromActiveImageCaptureList:framebuffer];
}
...
- (CGImageRef)newCGImageFromFramebufferContents;
{
    ...
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([RDGPUImageContext supportsFastTextureUpload])
        {
            ...
            dataProvider = CGDataProviderCreateWithData((__bridge_retained void*)self, rawImagePixels, paddedBytesForImage, RDDataProviderUnlockCallback);
            [[RDGPUImageContext sharedFramebufferCache] addFramebufferToActiveImageCaptureList:self]; // In case the framebuffer is swapped out on the filter, need to have a strong reference to it somewhere for it to hang on while the image is in existence
#else
#endif
        }
        else
        {
            ...
            dataProvider = CGDataProviderCreateWithData(NULL, rawImagePixels, totalBytesForImage, RDDataProviderReleaseCallback);
...
- (void)unlockAfterReading
{
    ...
    if ([RDGPUImageContext supportsFastTextureUpload])
    {
        NSAssert(readLockCount > 0, @"Unbalanced call to -[RDGPUImageFramebuffer unlockAfterReading]");
        readLockCount--;
        if (readLockCount == 0)
        {
