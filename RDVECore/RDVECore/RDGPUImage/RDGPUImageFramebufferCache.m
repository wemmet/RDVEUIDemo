...
@interface RDGPUImageFramebufferCache()
{
...
@implementation RDGPUImageFramebufferCache
...
//    framebufferCache = [[NSCache alloc] init];
    ...
    framebufferCacheQueue = dispatch_queue_create("com.sunsetlakesoftware.RDGPUImage.framebufferCacheQueue", RDGPUImageDefaultQueueAttribute());
...
//    activeImageCaptureList = [[NSMutableArray alloc] init];
    ...
    //NSLog(@"framebufferCache:%lu",(unsigned long)framebufferCache.count);
    //NSLog(@"framebufferTypeCounts:%lu",(unsigned long)framebufferTypeCounts.count);
 
    
    __block RDGPUImageFramebuffer *framebufferFromCache = nil;
//    dispatch_sync(framebufferCacheQueue, ^{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([numberOfMatchingTexturesInCache integerValue] < 1)
        {
            ...
            framebufferFromCache = [[RDGPUImageFramebuffer alloc] initWithSize:framebufferSize textureOptions:textureOptions onlyTexture:onlyTexture];
        }
        else
        {
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
        CVOpenGLESTextureCacheFlush([[RDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], 0);
#else
#endif
    });
}
