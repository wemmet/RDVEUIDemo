...
#if 1
        context = [[BCMeshTransformView alloc] getCurrentContext];
#else
        context = [EAGLContext currentContext];
        if (context) {
            context = [[EAGLContext alloc] initWithAPI:[context API] sharegroup:[context sharegroup]];
        }
        if (!context) {
            context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2];
        }
...
- (EAGLContext*)getCurrentContext
{
    ...
    if (_oldContext) {
        return [[EAGLContext alloc] initWithAPI:[_oldContext API] sharegroup:[_oldContext sharegroup]];
    }
    else
...
{
#if 1
//    if ([EAGLContext currentContext] == [BCMeshTransformView renderingContext]) {
        if (_oldContext) {//如果在其它地方(其它sdk)已经创建了EAGLContext，释放的时候应该把当前上下文设置为原来的，否则原来的将无法刷新
...
- (UIImage *)getGLViewImage
{
    return _glkView.snapshot;
}
...
//    NSLog(@"meshview %p",[BCMeshTransformView renderingContext]);
    _drawEnd = YES;
...
//    [self.glkView setNeedsDisplay];
    [self glkView:self.glkView drawInRect:self.contentView.frame];
}
