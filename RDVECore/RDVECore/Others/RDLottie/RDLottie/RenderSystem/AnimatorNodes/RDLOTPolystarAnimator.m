...
const CGFloat kRDPOLYSTAR_MAGIC_NUMBER = .47829f;

@implementation RDLOTPolystarAnimator {
  RDLOTNumberInterpolator *_outerRadiusInterpolator;
  RDLOTNumberInterpolator *_innerRadiusInterpolator;
  RDLOTNumberInterpolator *_outerRoundnessInterpolator;
  RDLOTNumberInterpolator *_innerRoundnessInterpolator;
...
- (instancetype _Nonnull)initWithInputNode:(RDLOTAnimatorNode *_Nullable)inputNode
  ...
  if (self) {
    ...
    _innerRadiusInterpolator = [[RDLOTNumberInterpolator alloc] initWithKeyframes:shapeStar.innerRadius.keyframes];
    _outerRoundnessInterpolator = [[RDLOTNumberInterpolator alloc] initWithKeyframes:shapeStar.outerRoundness.keyframes];
    _innerRoundnessInterpolator = [[RDLOTNumberInterpolator alloc] initWithKeyframes:shapeStar.innerRoundness.keyframes];
...
- (void)performLocalUpdate {
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    if (innerRoundness == 0 && outerRoundness == 0) {
      ...
      CGFloat cp1x = cp1Radius * cp1Roundedness * kRDPOLYSTAR_MAGIC_NUMBER * cp1Dx;
      CGFloat cp1y = cp1Radius * cp1Roundedness * kRDPOLYSTAR_MAGIC_NUMBER * cp1Dy;
      CGFloat cp2x = cp2Radius * cp2Roundedness * kRDPOLYSTAR_MAGIC_NUMBER * cp2Dx;
      CGFloat cp2y = cp2Radius * cp2Roundedness * kRDPOLYSTAR_MAGIC_NUMBER * cp2Dy;
      if (partialPointAmount != 0) {
        if (i == 0) {
