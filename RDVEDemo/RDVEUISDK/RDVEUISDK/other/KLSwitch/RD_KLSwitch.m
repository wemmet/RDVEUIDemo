...
typedef enum {
    RD_KLSwitchThumbJustifyLeft,
    RD_KLSwitchThumbJustifyRight
} RD_KLSwitchThumbJustify;

@interface RD_KLSwitchThumb : UIView
@property (nonatomic, assign) BOOL isTracking;
-(void) growThumbWithJustification:(RD_KLSwitchThumbJustify) justification;
-(void) shrinkThumbWithJustification:(RD_KLSwitchThumbJustify) justification;
@end

@interface RD_KLSwitchTrack : UIView
...
@interface RD_KLSwitch () <UIGestureRecognizerDelegate>{
}
@property (nonatomic, weak) id<RD_KLSwitchDelegate> delegate;
@property (nonatomic, strong) RD_KLSwitchTrack* track;
@property (nonatomic, strong) RD_KLSwitchThumb* thumb;
...
-(void) configureSwitch {
    ...
    // Initialization code
    if (!_track) {
        _track = [[RD_KLSwitchTrack alloc] initWithFrame: self.bounds
    ...
    if (!_thumb) {
        _thumb = [[RD_KLSwitchThumb alloc] initWithFrame:CGRectMake(kThumbOffset, kThumbOffset, self.bounds.size.height - 2 * kThumbOffset, self.bounds.size.height - 2 * kThumbOffset)];
...
-(void) setThumbIsTracking:(BOOL)isTracking {
    if (isTracking) {
        //Grow
        [self.thumb growThumbWithJustification: self.isOn ? RD_KLSwitchThumbJustifyRight : RD_KLSwitchThumbJustifyLeft];
    }
    else {
        //Shrink
        [self.thumb shrinkThumbWithJustification: self.isOn ? RD_KLSwitchThumbJustifyRight : RD_KLSwitchThumbJustifyLeft];
    }
...
@implementation RD_KLSwitchThumb//http://fir.im/sq55
...
-(void) growThumbWithJustification:(RD_KLSwitchThumbJustify) justification {
    ...
    if (justification == RD_KLSwitchThumbJustifyRight) {
...
@interface RD_KLSwitchTrack ()
@property (nonatomic, strong) UIView* contrastView;
@property (nonatomic, strong) UIView* onView;
@end

@implementation RD_KLSwitchTrack
