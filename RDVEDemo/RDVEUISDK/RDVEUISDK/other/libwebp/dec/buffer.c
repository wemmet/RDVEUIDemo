...
int RDWebPInitDecBufferInternal(WebPDecBuffer* buffer, int version) {
...
void WebPFreeDecBuffer(WebPDecBuffer* buffer) {
  if (buffer != NULL) {
//    if (!buffer->is_external_memory && buffer->private_memory) {
//      WebPSafeFree(buffer->private_memory);//有时会崩溃，注释掉后内存也不会增？？？
//    }
