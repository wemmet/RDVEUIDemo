...
int VP8RecordCoeffTokens(const int ctx, const int coeff_type,
  ...
  while (n < 16) {
    ...
    if (!AddToken(tokens, v != 0, base_id + 1)) {
      base_id = TOKEN_ID(coeff_type, RDVP8EncBands[n], 0);  // ctx=0
      continue;
    }
    if (!AddToken(tokens, v > 1, base_id + 2)) {
      base_id = TOKEN_ID(coeff_type, RDVP8EncBands[n], 1);  // ctx=1
    } else {
      ...
      base_id = TOKEN_ID(coeff_type, RDVP8EncBands[n], 2);  // ctx=2
    }
