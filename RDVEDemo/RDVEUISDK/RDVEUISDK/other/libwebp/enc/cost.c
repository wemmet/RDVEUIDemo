...
void VP8CalculateLevelCosts(RDVP8EncProba* const proba) {
  int ctype, band, ctx;
    ...
    for (n = 0; n < 16; ++n) {    // replicate bands. We don't need to sentinel.
      for (ctx = 0; ctx < NUM_CTX; ++ctx) {
        proba->remapped_costs_[ctype][n][ctx] =
            proba->level_cost_[ctype][RDVP8EncBands[n]][ctx];
      }
    }
  }
  proba->dirty_ = 0;
...
const uint16_t RDVP8FixedCostsUV[4] = { 302, 984, 439, 642 };
// note: these values include the fixed VP8BitCost(1, 145) mode selection cost.
const uint16_t RDVP8FixedCostsI16[4] = { 663, 919, 872, 919 };
const uint16_t RDVP8FixedCostsI4[NUM_BMODES][NUM_BMODES][NUM_BMODES] = {
...
void VP8InitResidual(int first, int coeff_type,
                     RDVP8Encoder* const enc, VP8Residual* const res) {
...
int VP8RecordCoeffs(int ctx, const VP8Residual* const res) {
  int n = res->first;
  // should be stats[RDVP8EncBands[n]], but it's equivalent for n=0 or 1
  ...
  while (n <= res->last) {
    ...
    while ((v = res->coeffs[n++]) == 0) {
      Record(0, s + 1);
      s = res->stats[RDVP8EncBands[n]][0];
    }
    Record(1, s + 1);
    if (!Record(2u < (unsigned int)(v + 1), s + 2)) {  // v = -1 or 1
      s = res->stats[RDVP8EncBands[n]][1];
    } else {
      v = abs(v);
