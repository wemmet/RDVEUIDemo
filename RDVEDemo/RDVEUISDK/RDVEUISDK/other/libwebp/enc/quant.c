...
static void PrintBlockInfo(const RDVP8EncIterator* const it,
...
static void SetupMatrices(RDVP8Encoder* enc) {
  int i;
...
static void SetupFilterStrength(RDVP8Encoder* const enc) {
  int i;
...
static void SimplifySegments(RDVP8Encoder* const enc) {
...
static int TrellisQuantizeBlock(const RDVP8Encoder* const enc,
...

  {
    ...
    const int last_proba = probas[RDVP8EncBands[first]][ctx0][0];
  ...
  for (n = first; n <= last; ++n) {
    ...
    for (m = -MIN_DELTA; m <= MAX_DELTA; ++m) {
      ...
      const int band = RDVP8EncBands[n + 1];
...
static int ReconstructIntra16(RDVP8EncIterator* const it,
                              VP8ModeScore* const rd,
                              uint8_t* const yuv_out,
                              int mode) {
  const RDVP8Encoder* const enc = it->enc_;
  ...
  nz |= RDVP8EncQuantizeBlockWHT(dc_tmp, rd->y_dc_levels, &dqm->y2_) << 24;
  ...
  if (DO_TRELLIS_I16 && it->do_trellis_) {
    ...
    for (n = 0; n < 16; n += 2) {
      ...
      tmp[n][0] = tmp[n + 1][0] = 0;
      nz |= RDVP8EncQuantize2Blocks(tmp[n], rd->y_ac_levels[n], &dqm->y1_) << n;
...
static int ReconstructIntra4(RDVP8EncIterator* const it,
                             int16_t levels[16],
  ...
  if (DO_TRELLIS_I4 && it->do_trellis_) {
    ...
    nz = RDVP8EncQuantizeBlock(tmp, levels, &dqm->y1_);
  }
  VP8ITransform(ref, tmp, yuv_out, 0);
  return nz;
}

static int ReconstructUV(RDVP8EncIterator* const it, VP8ModeScore* const rd,
  ...
  if (DO_TRELLIS_UV && it->do_trellis_) {
    ...
    for (n = 0; n < 8; n += 2) {
      nz |= RDVP8EncQuantize2Blocks(tmp[n], rd->uv_levels[n], &dqm->uv_) << n;
    }
  }
...
static void SwapOut(RDVP8EncIterator* const it) {
...
static void PickBestIntra16(RDVP8EncIterator* const it, VP8ModeScore* rd) {
  ...
  for (mode = 0; mode < NUM_PRED_MODES; ++mode) {
    ...
    rd_cur->H = RDVP8FixedCostsI16[mode];
...
static const uint16_t* GetCostModeI4(RDVP8EncIterator* const it,
                                     const uint8_t modes[16]) {
  const int preds_w = it->enc_->preds_w_;
  const int x = (it->i4_ & 3), y = it->i4_ >> 2;
  const int left = (x == 0) ? it->preds_[y * preds_w - 1] : modes[it->i4_ - 1];
  const int top = (y == 0) ? it->preds_[-preds_w + x] : modes[it->i4_ - 4];
  return RDVP8FixedCostsI4[top][left];
}

static int PickBestIntra4(RDVP8EncIterator* const it, VP8ModeScore* const rd) {
  const RDVP8Encoder* const enc = it->enc_;
  const VP8SegmentInfo* const dqm = &enc->dqm_[it->mb_->segment_];
...
static void PickBestUV(RDVP8EncIterator* const it, VP8ModeScore* const rd) {
  ...
  for (mode = 0; mode < NUM_PRED_MODES; ++mode) {
    ...
    rd_uv.H  = RDVP8FixedCostsUV[mode];
...
static void SimpleQuantize(RDVP8EncIterator* const it, VP8ModeScore* const rd) {
  const RDVP8Encoder* const enc = it->enc_;
  const int is_i16 = (it->mb_->type_ == 1);
  int nz = 0;
...
static void RefineUsingDistortion(RDVP8EncIterator* const it,
