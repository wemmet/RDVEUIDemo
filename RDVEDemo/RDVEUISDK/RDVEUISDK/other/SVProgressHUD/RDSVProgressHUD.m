...
#import "RDSVIndefiniteAnimatedView.h"
#import "RDSVProgressAnimatedView.h"
#import "RDSVRadialGradientLayer.h"

NSString * const RDSVProgressHUDDidReceiveTouchEventNotification = @"SVProgressHUDDidReceiveTouchEventNotification";
NSString * const RDSVProgressHUDDidTouchDownInsideNotification = @"SVProgressHUDDidTouchDownInsideNotification";
NSString * const RDSVProgressHUDWillDisappearNotification = @"SVProgressHUDWillDisappearNotification";
NSString * const RDSVProgressHUDDidDisappearNotification = @"SVProgressHUDDidDisappearNotification";
NSString * const RDSVProgressHUDWillAppearNotification = @"SVProgressHUDWillAppearNotification";
NSString * const RDSVProgressHUDDidAppearNotification = @"SVProgressHUDDidAppearNotification";

NSString * const RDSVProgressHUDStatusUserInfoKey = @"SVProgressHUDStatusUserInfoKey";

static const CGFloat RDSVProgressHUDParallaxDepthPoints = 10;
static const CGFloat RDSVProgressHUDUndefinedProgress = -1;
static const CGFloat RDSVProgressHUDDefaultAnimationDuration = 0.15;

@interface RDSVProgressHUD ()
...
@property (nonatomic, strong) RDSVProgressAnimatedView *ringView;
@property (nonatomic, strong) RDSVProgressAnimatedView *backgroundRingView;
...
- (RDSVProgressAnimatedView*)ringView;
- (RDSVProgressAnimatedView*)backgroundRingView;
...
@implementation RDSVProgressHUD {
    BOOL _isInitializing;
}

+ (RDSVProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static RDSVProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(RDSVProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(RDSVProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(RDSVProgressHUDAnimationType)type {
...
+ (void)showWithStatus:(NSString*)status {
    [self sharedView];
    [self showProgress:RDSVProgressHUDUndefinedProgress status:status];
}
...
+ (void)dismissWithSuccess:(NSString*)successString {
    [[self sharedView] dismissWithStatus:successString error:NO];
}
...
+ (void)dismissWithError:(NSString*)errorString {
    [[self sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds {
    [[self sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = RDSVProgressHUDMaskTypeNone;
        _defaultStyle = RDSVProgressHUDStyleLight;
        _defaultAnimationType = RDSVProgressHUDAnimationTypeNative;
...
#if 0
        NSBundle *bundle = [NSBundle bundleForClass:[RDSVProgressHUD class]];
        NSURL *url = [bundle URLForResource:@"RDSVProgressHUD" withExtension:@"bundle"];
...
#endif
        _ringThickness = 2.0f;
        _ringRadius = 0.0f;//18.0f;
        _ringNoTextRadius = 24.0f;
        
        _cornerRadius = 1.0f;//14.0f;
        
        _minimumDismissTimeInterval = 5.0;

        _fadeInAnimationDuration = RDSVProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = RDSVProgressHUDDefaultAnimationDuration;
        
        _maxSupportedWindowLevel = UIWindowLevelNormal;
        
        // Accessibility support
        self.accessibilityIdentifier = @"RDSVProgressHUD";
        self.accessibilityLabel = @"RDSVProgressHUD";
...
//    [self updateBlurBounds];
    ...
    if(self.progress != RDSVProgressHUDUndefinedProgress) {
...
- (void)updateMask {
    ...
    switch (self.defaultMaskType) {
        case RDSVProgressHUDMaskTypeCustom:
        case RDSVProgressHUDMaskTypeBlack:{
            
            self.backgroundLayer = [CALayer layer];
            self.backgroundLayer.frame = self.bounds;
            self.backgroundLayer.backgroundColor = self.defaultMaskType == RDSVProgressHUDMaskTypeCustom ? self.backgroundLayerColor.CGColor : [UIColor colorWithWhite:0 alpha:0.4].CGColor;
        ...
        case RDSVProgressHUDMaskTypeGradient:{
            RDSVRadialGradientLayer *layer = [RDSVRadialGradientLayer layer];
...
- (void)updateBlurBounds {
    ...
    if(NSClassFromString(@"UIBlurEffect") && self.defaultStyle != RDSVProgressHUDStyleCustom) {
        ...
        if(self.backgroundColorForStyle != [UIColor clearColor]) {
            // Create blur effect
            UIBlurEffectStyle blurEffectStyle = self.defaultStyle == RDSVProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    if([self.hudView respondsToSelector:@selector(addMotionEffect:)]) {
        ...
        effectX.minimumRelativeValue = @(-RDSVProgressHUDParallaxDepthPoints);
        effectX.maximumRelativeValue = @(RDSVProgressHUDParallaxDepthPoints);
        
        UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
        effectY.minimumRelativeValue = @(-RDSVProgressHUDParallaxDepthPoints);
        effectY.maximumRelativeValue = @(RDSVProgressHUDParallaxDepthPoints);
...
- (NSDictionary*)notificationUserInfo{
    return (self.statusLabel.text ? @{RDSVProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        ...
        __weak RDSVProgressHUD *weakSelf = self;
        [UIView animateWithDuration:animationDuration
                         ...
                         animations:^{
                             __strong RDSVProgressHUD *strongSelf = weakSelf;
                             if(strongSelf) {
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = RDSVProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != RDSVProgressHUDMaskTypeNone) {
    ...
    if(self.alpha != 1.0f || self.hudView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(strongSelf) {
                ...
                if(strongSelf.alpha == 1.0f && strongSelf.hudView.alpha == 1.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDDidAppearNotification
                                                                        object:strongSelf
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    ...
    [self performSelector:@selector(dismissDelay) withObject:nil afterDelay:seconds];
}
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(RDSVProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDWillDisappearNotification
                                                                object:nil
                                                              userInfo:[strongSelf notificationUserInfo]];
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.alpha == 0.0f && strongSelf.hudView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:RDSVProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == RDSVProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[RDSVIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[RDSVIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        RDSVIndefiniteAnimatedView *indefiniteAnimatedView = (RDSVIndefiniteAnimatedView*)_indefiniteAnimatedView;
        ...
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        }
...
- (RDSVProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[RDSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (RDSVProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[RDSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == RDSVProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == RDSVProgressHUDStyleDark) {
...
- (BOOL)isClear { // used for iOS 7 and above
    return (self.defaultMaskType == RDSVProgressHUDMaskTypeClear || self.defaultMaskType == RDSVProgressHUDMaskTypeNone);
}
...
- (void)setDefaultAnimationType:(RDSVProgressHUDAnimationType)animationType {
