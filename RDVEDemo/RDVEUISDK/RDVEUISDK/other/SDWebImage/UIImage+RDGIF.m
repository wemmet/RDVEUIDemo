...
+ (UIImage *)rd_sd_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self rd_sd_frameDurationAtIndex:i source:source];
...
+ (float)rd_sd_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)rd_sd_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage rd_sd_animatedGIFWithData:data];
        }
        ...
        return [UIImage imageNamed:[NSString stringWithFormat:@"RDVEUISDK.bundle/%@",name]];
    }
    else {
        NSString *path = [[NSBundle mainBundle] pathForResource:name ofType:@"gif"];
