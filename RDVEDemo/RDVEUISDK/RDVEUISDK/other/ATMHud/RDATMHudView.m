...
#import "RDATMHudView.h"
#import "RDATMTextLayer.h"
#import "RDATMProgressLayer.h"
#import "RDATMHud.h"
...
- (id)initWithFrame:(CGRect)frame andController:(RDATMHud *)c {
    if ((self = [super initWithFrame:frame])) {
		...
		captionLayer = [[RDATMTextLayer alloc] init];
		...
		progressLayer = [[RDATMProgressLayer alloc] init];
...
- (void)dealloc {
    ...
    imageLayer = nil;
    if(captionLayer.superlayer)
        [captionLayer removeFromSuperlayer];
    captionLayer = nil;
    if(progressLayer.superlayer)
...
- (void)calculate {
	if (!caption || [caption isEqualToString:@""]) {
        ...
        CGSize captionSize = [caption boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 40, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:14]} context:nil].size;
		...
		if (!hasFixedSize) {
			...
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom) {
				...
				if (marginX < p.margin) {
                    captionSize = [caption boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 40, 200) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:14]} context:nil].size;
					captionRect.size = captionSize;
...
- (CGSize)calculateSizeForQueueItem:(RDATMHudQueueItem *)item {
	...
	if (!item.caption || [item.caption isEqualToString:@""]) {
		...
		BOOL hasFixedSize = NO;
        CGSize captionSize = [caption boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 40, 200) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:14]} context:nil].size;
...
- (void)applyWithMode:(RDATMHudApplyMode)mode {
    ...
    __weak RDATMHudView *weakself = self;

    switch (mode) {
		case RDATMHudApplyModeShow: {
			if (CGPointEqualToPoint(p.center, CGPointZero)) {
                ...
                self.frame = CGRectMake(([UIScreen mainScreen].bounds.size.width-targetBounds.size.width)*0.5, ([UIScreen mainScreen].bounds.size.height-targetBounds.size.height)*0.5, targetBounds.size.width, targetBounds.size.height);
			...
			[CATransaction setCompletionBlock:^{
				if (showActivity) {
					...
					activity.frame = [weakself sharpRect:activityRect];
				}
				
				CGRect r = weakself.frame;
				[weakself setFrame:[weakself sharpRect:r]];
				...
				weakself.transform = CGAffineTransformMakeScale(p.appearScaleFactor, p.appearScaleFactor);
		...
		case RDATMHudApplyModeHide: {
			...
			[UIView animateWithDuration:.1 
							 animations:^{ 
								 weakself.alpha = 0.0;
								 weakself.transform = CGAffineTransformMakeScale(p.disappearScaleFactor, p.disappearScaleFactor);
							 } 
							 completion:^(BOOL finished){
								 if (finished) {
...
- (void)show {
	[self calculate];
	[self applyWithMode:RDATMHudApplyModeShow];
}

- (void)hide {
	[self applyWithMode:RDATMHudApplyModeHide];
}

- (void)update {
	[self calculate];
	[self applyWithMode:RDATMHudApplyModeUpdate];
}
