...
#import "RDATMSoundFX.h"
#import "RDATMHudQueueItem.h"

@interface RDATMHud (Private)
- (void)construct;
@end

@implementation RDATMHud
...
- (void)showQueueAtIndex:(NSInteger)index {
	if ([displayQueue count] > 0) {
		...
		RDATMHudQueueItem *item = [displayQueue objectAtIndex:queuePosition];
...
- (void)construct {
	...
	__view = [[RDATMHudView alloc] initWithFrame:CGRectZero andController:self];
	...
	if (!blockTouches) {
		...
		if (aTouch.tapCount == 1) {
			...
			if (CGRectContainsPoint(__view.frame, p)) {
                if( delegate){
                    if ([(id)delegate respondsToSelector:@selector(userDidTapHud:)]) {
                        [delegate userDidTapHud:self];
                    }
                }
			}
		}
...
- (void)playSound:(NSString *)soundPath {
	sound = [[RDATMSoundFX alloc] initWithContentsOfFile:soundPath];
	[sound play];
}
