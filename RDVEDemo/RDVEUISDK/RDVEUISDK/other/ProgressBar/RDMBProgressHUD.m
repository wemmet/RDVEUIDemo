...
@implementation RDMBProgressHUD
...
- (void)setMode:(RDMBProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == RDMBProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(RDMBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == RDMBProgressHUDModeDeterminate) {
        self.indicator = [[[RDMBRoundProgressView alloc] init] autorelease];
    }
    else if (mode == RDMBProgressHUDModeCustomView && self.customView != nil){
...
+ (RDMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	RDMBProgressHUD *hud = [[[RDMBProgressHUD alloc] initWithView:view] autorelease];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[RDMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		RDMBProgressHUD *HUD = (RDMBProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"RDMBProgressHUDViewIsNillException" 
					format:@"The view used in the RDMBProgressHUD initializer is nil."];
	}
	id me = [self initWithFrame:view.bounds];
	// We need to take care of rotation ourselfs if we're adding the HUD to a window
	if ([view isKindOfClass:[UIWindow class]]) {
		[me setTransformForCurrentOrientation:NO];
	}
	[[NSNotificationCenter defaultCenter] addObserver:me selector:@selector(deviceOrientationDidChange:)
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = RDMBProgressHUDAnimationFade;
        self.mode = RDMBProgressHUDModeIndeterminate;
        self.labelText = nil;
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
			...
			CGSize labelSize = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight)
                                                         ...
                                                         attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:detailsLabel.font.pointSize]}
                                                            context:nil].size;
    ...
    if(delegate != nil) {
        if(delegate){
            if ([delegate respondsToSelector:@selector(cancelDownLoad)]) {
                [delegate performSelector:@selector(cancelDownLoad) withObject:self];
            }
        }
    }
}
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == RDMBProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == RDMBProgressHUDAnimationZoom) {
...
@implementation RDMBRoundProgressView
