...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"RDProgressHUD needs to be accessed on the main thread.");

CGFloat const RDProgressMaxOffset = 1000000.f;
...
@interface RDProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    RDProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    RDProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (RDProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (RDProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = RDProgressHUDAnimationFade;
    _mode = RDProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(RDProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == RDProgressHUDAnimationZoom) {
        type = animatingIn ? RDProgressHUDAnimationZoomIn : RDProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == RDProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == RDProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == RDProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == RDProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    RDProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<RDProgressHUDDelegate> delegate = _delegate;
    if(_delegate){
...
- (void)setupViews {
    ...
    RDBackgroundView *backgroundView = [[RDBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = RDProgressHUDBackgroundStyleSolidColor;
    ...
    RDBackgroundView *bezelView = [RDBackgroundView new];
...
#endif
    UIButton *button = [RDProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[RDRoundProgressView class]];
    
    RDProgressHUDMode mode = self.mode;
    if (mode == RDProgressHUDModeIndeterminate) {
    ...
    else if (mode == RDProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[RDBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == RDProgressHUDModeDeterminate || mode == RDProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[RDRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == RDProgressHUDModeAnnularDeterminate) {
            [(RDRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == RDProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == RDProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[RDProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[RDProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[RDRoundProgressView class]]) {
        RDRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [RDRoundProgressView appearanceWhenContainedIn:[RDProgressHUD class], nil];
#else
        appearance = [RDRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[RDProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((RDRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((RDRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[RDBarProgressView class]]) {
        RDBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [RDBarProgressView appearanceWhenContainedIn:[RDProgressHUD class], nil];
#else
        appearance = [RDBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[RDProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((RDBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((RDBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    RDBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(RDProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == RDProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation RDRoundProgressView
...
@implementation RDBarProgressView
...
@implementation RDBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = RDProgressHUDBackgroundStyleBlur;
            ...
            _style = RDProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(RDProgressHUDBackgroundStyle)style {
    if (style == RDProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = RDProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    RDProgressHUDBackgroundStyle style = self.style;
    if (style == RDProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == RDProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation RDProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [RDProgressHUD allHUDsForView:view];
    for (RDProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable RDProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    RDBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == RDProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = RDProgressHUDBackgroundStyleSolidColor;
...
@implementation RDProgressHUDRoundedButton
