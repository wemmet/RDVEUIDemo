...
typedef NS_ENUM(NSUInteger, RDYYWebImageFromType) {
    ...
    RDYYWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    RDYYWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    RDYYWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    RDYYWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, RDYYWebImageStage) {
    
    /// Incomplete, progressive image.
    RDYYWebImageStageProgress  = -1,
    
    /// Cancelled.
    RDYYWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    RDYYWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^RDYYWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);

/**
 The block invoked before remote image fetch finished to do additional image process.
 
 @discussion This block will be invoked before `RDYYWebImageCompletionBlock` to give
 ...
 */
typedef UIImage * _Nullable (^RDYYWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^RDYYWebImageCompletionBlock)(UIImage * _Nullable image,
                                          NSURL *url,
                                          RDYYWebImageFromType from,
                                          RDYYWebImageStage stage,
 ...
 */
@interface RDYYWebImageManager : NSObject

/**
 Returns global RDYYWebImageManager instance.
 
 @return RDYYWebImageManager shared instance.
 */
 ...
 */
- (instancetype)initWithCache:(nullable RDYYImageCache *)cache
 ...
 */
- (nullable RDYYWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) RDYYImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) RDYYWebImageTransformBlock sharedTransformBlock;
...
/**
 ...
 @param url A specified URL
 @return Cache key used in RDYYImageCache.
 */
