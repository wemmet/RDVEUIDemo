//
//  RDYYKVStorage.h
//  RDYYCache <https://github.com/ibireme/RDYYCache>
//
...
/**
 RDYYKVStorageItem is used by `RDYYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface RDYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `RDYYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use RDYYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use RDYYKVStorageTypeFile to get better performance.
 * You can use RDYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, RDYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    RDYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    RDYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    RDYYKVStorageTypeMixed = 2,
};



/**
 RDYYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for RDYYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface RDYYKVStorage : NSObject
...
@property (nonatomic, readonly) RDYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(RDYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is RDYYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is RDYYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is RDYYKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(RDYYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 RDYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is RDYYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is RDYYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is RDYYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable RDYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable RDYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `RDYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<RDYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `RDYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<RDYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
