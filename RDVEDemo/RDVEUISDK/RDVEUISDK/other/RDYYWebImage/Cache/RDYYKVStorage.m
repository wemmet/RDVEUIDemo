...
@implementation RDYYKVStorageItem
@end

@implementation RDYYKVStorage {
...
- (RDYYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    RDYYKVStorageItem *item = [RDYYKVStorageItem new];
...
- (RDYYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    RDYYKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            RDYYKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"RDYYKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:RDYYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(RDYYKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"RDYYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > RDYYKVStorageTypeMixed) {
        NSLog(@"RDYYKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"RDYYKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"RDYYKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_RDYYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_RDYYSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == RDYYKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != RDYYKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case RDYYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case RDYYKVStorageTypeFile:
        case RDYYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (RDYYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (RDYYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    RDYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (RDYYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    RDYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != RDYYKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            RDYYKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
