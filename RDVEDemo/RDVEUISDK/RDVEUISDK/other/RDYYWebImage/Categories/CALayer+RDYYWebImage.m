...
@interface CALayer_RDYYWebImage : NSObject @end
@implementation CALayer_RDYYWebImage @end


static int _RDYYWebImageSetterKey;

@implementation CALayer (RDYYWebImage)
...
- (void)yy_setImageWithURL:(NSURL *)imageURL
    ...
    _yy_dispatch_sync_on_main_queue(^{
        if ((options & RDYYWebImageOptionSetImageWithFadeAnimation) &&
            ...
            [self removeAnimationForKey:_RDYYWebImageFadeAnimationKey];
        }
        ...
        dispatch_async([_RDYYWebImageSetter setterQueue], ^{
            ...
            RDYYWebImageCompletionBlock _completion = ^(UIImage *image, NSURL *url, RDYYWebImageFromType from, RDYYWebImageStage stage, NSError *error) {
                ...
                BOOL showFade = (options & RDYYWebImageOptionSetImageWithFadeAnimation);
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (setImage && self && !sentinelChanged) {
                        if (showFade) {
                            ...
                            [self addAnimation:transition forKey:_RDYYWebImageFadeAnimationKey];
                        }
