...
static BOOL RDYYCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface _RDYYWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _RDYYWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_RDYYWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface RDYYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) RDYYImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) RDYYWebImageProgressBlock progress;
@property (nonatomic, copy) RDYYWebImageTransformBlock transform;
@property (nonatomic, copy) RDYYWebImageCompletionBlock completion;
@end


@implementation RDYYWebImageOperation
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"RDYYWebImageOperation init error" reason:@"RDYYWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if (_taskID != UIBackgroundTaskInvalid) {
        [_RDYYSharedApplication() endBackgroundTask:_taskID];
        _taskID = UIBackgroundTaskInvalid;
    }
    if ([self isExecuting]) {
        self.cancelled = YES;
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & RDYYWebImageOptionShowNetworkActivity)) {
                [RDYYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, RDYYWebImageFromNone, RDYYWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & RDYYWebImageOptionUseNSURLCache) &&
            !(_options & RDYYWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:RDYYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, RDYYWebImageFromMemoryCache, RDYYWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & RDYYWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:RDYYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:RDYYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & RDYYWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, RDYYWebImageFromNone, RDYYWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[_RDYYWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & RDYYWebImageOptionShowNetworkActivity)) {
                [RDYYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, RDYYWebImageFromNone, RDYYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, RDYYWebImageFromDiskCache, RDYYWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & RDYYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([RDYYWebImageOperation _imageQueue], ^{
                        RDYYImageCacheType cacheType = (_options & RDYYWebImageOptionIgnoreDiskCache) ? RDYYImageCacheTypeMemory : RDYYImageCacheTypeAll;
            ...
            if (!image) {
                ...
                if (_options & RDYYWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, RDYYWebImageFromRemote, RDYYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & RDYYWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & RDYYWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & RDYYWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & RDYYWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[RDYYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == RDYYImageTypeUnknown ||
            _progressiveDecoder.type == RDYYImageTypeWebP ||
            _progressiveDecoder.type == RDYYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != RDYYImageTypeJPEG &&
                _progressiveDecoder.type != RDYYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            RDYYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, RDYYWebImageFromRemote, RDYYWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == RDYYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!RDYYCGImageLastPixelFilled(image.CGImage)) return;
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, RDYYWebImageFromRemote, RDYYWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & RDYYWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & RDYYWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[RDYYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((RDYYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    RDYYImageDecoder *decoder = [RDYYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                RDYYImageType imageType = RDYYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case RDYYImageTypeJPEG:
                    case RDYYImageTypeGIF:
                    case RDYYImageTypePNG:
                    case RDYYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == RDYYImageTypeGIF ||
                                imageType == RDYYImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & RDYYWebImageOptionShowNetworkActivity)) {
                [RDYYWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, RDYYWebImageFromNone, RDYYWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & RDYYWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & RDYYWebImageOptionAllowBackgroundTask) && _RDYYSharedApplication()) {
                    __weak __typeof__ (self) _self = self;
                    if (_taskID == UIBackgroundTaskInvalid) {
                        _taskID = [_RDYYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{
