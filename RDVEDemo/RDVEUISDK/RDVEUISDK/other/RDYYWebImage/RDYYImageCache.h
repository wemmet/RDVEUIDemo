...
@class RDYYMemoryCache, RDYYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, RDYYImageCacheType) {
    /// No value.
    RDYYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    RDYYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    RDYYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    RDYYImageCacheTypeAll    = RDYYImageCacheTypeMemory | RDYYImageCacheTypeDisk,
};


/**
 RDYYImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface RDYYImageCache : NSObject
...
/** The underlying memory cache. see `RDYYMemoryCache` for more information.*/
@property (strong, readonly) RDYYMemoryCache *memoryCache;

/** The underlying disk cache. see `RDYYDiskCache` for more information.*/
@property (strong, readonly) RDYYDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'RDYYImage' to decode 
...
/**
 ...
 @return  The singleton RDYYImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `RDYYImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `RDYYImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(RDYYImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `RDYYImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(RDYYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(RDYYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, RDYYImageCacheType type))block;
