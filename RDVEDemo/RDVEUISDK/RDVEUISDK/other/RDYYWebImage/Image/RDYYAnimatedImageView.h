...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `RDYYAnimatedImage` protocol,
...
/**
 The RDYYAnimatedImage protocol declares the required methods for animated image
 display with RDYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to RDYYAnimatedImageView.image or RDYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `RDYYImage` and `RDYYFrameImage` for example.
 */
@protocol RDYYAnimatedImage <NSObject>
@required
