...
+ (RDYYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (RDYYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (RDYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        RDYYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"RDYYImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"RDYYImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"RDYYImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"RDYYImageData"];
    } else {
...
#pragma mark - protocol RDYYAnimatedImage
