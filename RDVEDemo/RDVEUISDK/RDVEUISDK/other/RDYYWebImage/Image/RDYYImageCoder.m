...
#import "webp/mux.h"
#else
#define RDYYIMAGE_WEBP_ENABLED 0
#endif
#endif
...
#define RDYY_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define RDYY_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))
...
typedef enum {
    RDYY_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    RDYY_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    RDYY_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} yy_png_alpha_type;

typedef enum {
    RDYY_PNG_DISPOSE_OP_NONE = 0,
    RDYY_PNG_DISPOSE_OP_BACKGROUND = 1,
    RDYY_PNG_DISPOSE_OP_PREVIOUS = 2,
} yy_png_dispose_op;

typedef enum {
    RDYY_PNG_BLEND_OP_SOURCE = 0,
    RDYY_PNG_BLEND_OP_OVER = 1,
} yy_png_blend_op;
...
static bool yy_png_validate_animation_chunk_order(yy_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != RDYY_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != RDYY_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        ...
        switch (chunk->fourcc) {
            case RDYY_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case RDYY_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != RDYY_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case RDYY_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case RDYY_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != RDYY_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != RDYY_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == RDYY_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case RDYY_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != RDYY_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != RDYY_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static yy_png_info *yy_png_info_create(const uint8_t *data, uint32_t length) {
    ...
    if (*((uint32_t *)data) != RDYY_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != RDYY_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    ...
    do {
        ...
        switch (chunk->fourcc) {
            ...
            case RDYY_FOUR_CC('f', 'c', 'T', 'L') :
            case RDYY_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == RDYY_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != RDYY_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                ...
                if (insert_chunk_info->fourcc == RDYY_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = yy_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = RDYY_FOUR_CC('I', 'D', 'A', 'T');
        ...
        if (shared_chunk_info->fourcc == RDYY_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
...
static inline size_t RDYYImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat RDYYImageDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef RDYYCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef RDYYCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL RDYYCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, RDYYCGColorSpaceGetDeviceRGB());
}

BOOL RDYYCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, RDYYCGColorSpaceGetDeviceGray());
}
...
/**
 ...
 CGDataProviderRef provider = CGDataProviderCreateWithData(data, data, size, RDYYCGDataProviderReleaseDataCallback);
 */
static void RDYYCGDataProviderReleaseDataCallback(void *info, const void *data, size_t size) {
    if (info) free(info);
}
 ...
 */
static BOOL RDYYCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
...
/**
 ...
 @return Whether succeed.
 */
static BOOL RDYYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = RDYYCGColorSpaceGetDeviceRGB();
    destFormat.bitmapInfo = bitmapInfo;
    dest->data = NULL;
    if (RDYYCGImageDecodeToBitmapBufferWithAnyFormat(srcImage, dest, &destFormat)) return YES;
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef RDYYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef RDYYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    if(!RDYYCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &src, kCGImageAlphaFirst | kCGBitmapByteOrderDefault)) return NULL;
    
    size_t destBytesPerRow = RDYYImageByteAlign(destWidth * 4, 32);
    ...
    tmpProvider = CGDataProviderCreateWithData(tmp.data, tmp.data, destHeight * destBytesPerRow, RDYYCGDataProviderReleaseDataCallback);
    if (!tmpProvider) goto fail;
    tmp.data = NULL; // hold by provider
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, RDYYCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    if (!RDYYCGImageDecodeToBitmapBufferWith32BitFormat(tmpImage, &dest, destBitmapInfo)) goto fail;
    CFRelease(tmpImage);
    tmpImage = NULL;
    
    destProvider = CGDataProviderCreateWithData(dest.data, dest.data, destHeight * destBytesPerRow, RDYYCGDataProviderReleaseDataCallback);
    if (!destProvider) goto fail;
    dest.data = NULL; // hold by provider
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, RDYYCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation RDYYUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger RDYYUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef RDYYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(RDYYImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(RDYYImageDegreesToRadians(90));
    ...
    return RDYYCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

RDYYImageType RDYYImageDetectType(CFDataRef data) {
    if (!data) return RDYYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return RDYYImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case RDYY_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return RDYYImageTypeTIFF;
        } break;
            
        case RDYY_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return RDYYImageTypeTIFF;
        } break;
            
        case RDYY_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return RDYYImageTypeICO;
        } break;
            
        case RDYY_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return RDYYImageTypeICO;
        } break;
            
        case RDYY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return RDYYImageTypeICNS;
        } break;
            
        case RDYY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return RDYYImageTypeGIF;
        } break;
            
        case RDYY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == RDYY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return RDYYImageTypePNG;
            }
        } break;
            
        case RDYY_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == RDYY_FOUR_CC('W', 'E', 'B', 'P')) {
                return RDYYImageTypeWebP;
            }
        } break;
        /*
        case RDYY_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return RDYYImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        case RDYY_TWO_CC('B', 'A'):
        case RDYY_TWO_CC('B', 'M'):
        case RDYY_TWO_CC('I', 'C'):
        case RDYY_TWO_CC('P', 'I'):
        case RDYY_TWO_CC('C', 'I'):
        case RDYY_TWO_CC('C', 'P'): { // BMP
            return RDYYImageTypeBMP;
        }
        case RDYY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return RDYYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return RDYYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return RDYYImageTypeJPEG2000;
    
    return RDYYImageTypeUnknown;
}

CFStringRef RDYYImageTypeToUTType(RDYYImageType type) {
    switch (type) {
        case RDYYImageTypeJPEG: return kUTTypeJPEG;
        case RDYYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case RDYYImageTypeTIFF: return kUTTypeTIFF;
        case RDYYImageTypeBMP: return kUTTypeBMP;
        case RDYYImageTypeICO: return kUTTypeICO;
        case RDYYImageTypeICNS: return kUTTypeAppleICNS;
        case RDYYImageTypeGIF: return kUTTypeGIF;
        case RDYYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

RDYYImageType RDYYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(RDYYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(RDYYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(RDYYImageTypeTIFF),
                (id)kUTTypeBMP : @(RDYYImageTypeBMP),
                (id)kUTTypeICO : @(RDYYImageTypeICO),
                (id)kUTTypeAppleICNS : @(RDYYImageTypeICNS),
                (id)kUTTypeGIF : @(RDYYImageTypeGIF),
                (id)kUTTypePNG : @(RDYYImageTypePNG)};
    });
    if (!uti) return RDYYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *RDYYImageTypeGetExtension(RDYYImageType type) {
    switch (type) {
        case RDYYImageTypeJPEG: return @"jpg";
        case RDYYImageTypeJPEG2000: return @"jp2";
        case RDYYImageTypeTIFF: return @"tiff";
        case RDYYImageTypeBMP: return @"bmp";
        case RDYYImageTypeICO: return @"ico";
        case RDYYImageTypeICNS: return @"icns";
        case RDYYImageTypeGIF: return @"gif";
        case RDYYImageTypePNG: return @"png";
        case RDYYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef RDYYCGImageCreateEncodedData(CGImageRef imageRef, RDYYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == RDYYImageTypeWebP) {
#if RDYYIMAGE_WEBP_ENABLED
        if (quality == 1) {
            return RDYYCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, RDYYImagePresetDefault);
        } else {
            return RDYYCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, RDYYImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = RDYYImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL RDYYImageWebPAvailable() {
    return YES;
}

CFDataRef RDYYCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, RDYYImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    if(!RDYYCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &buffer, kCGImageAlphaLast | kCGBitmapByteOrderDefault)) return nil;
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > RDYYImagePresetText ? RDYYImagePresetDefault : preset;
...
NSUInteger RDYYImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef RDYYCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = RDYYImageByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
    ...
    provider = CGDataProviderCreateWithData(destBytes, destBytes, destLength, RDYYCGDataProviderReleaseDataCallback);
    if (!provider) goto fail;
    destBytes = NULL; // hold by provider
    
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, RDYYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation RDYYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    RDYYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    RDYYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _RDYYImageDecoderFrame : RDYYImageFrame
...
@implementation _RDYYImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _RDYYImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation RDYYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    RDYYImageDecoder *decoder = [[RDYYImageDecoder alloc] initWithScale:scale];
...
- (RDYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    RDYYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_RDYYImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    RDYYImageType type = RDYYImageDetectType((__bridge CFDataRef)data);
...
- (RDYYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _RDYYImageDecoderFrame *frame = [(_RDYYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != RDYYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = RDYYCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == RDYYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = RDYYImageDisposeNone;
        frame.blend = RDYYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case RDYYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case RDYYImageTypePNG: {
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _RDYYImageDecoderFrame *frame = [_RDYYImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = RDYYImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = RDYYImageBlendOver;
            }
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/RDYYImage#installation", func, line);
    });
#endif
}
...
- (void)_updateSourceAPNG {
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        switch (fi->frame_control.dispose_op) {
            case RDYY_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = RDYYImageDisposeBackground;
            } break;
            case RDYY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = RDYYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = RDYYImageDisposeNone;
            } break;
        ...
        if (frame.blend == RDYYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != RDYYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == RDYYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == RDYYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == RDYYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = RDYYUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _RDYYImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = RDYYCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(bytes, bytes, size, RDYYCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(bytes);
            return NULL;
        }
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = RDYYImageByteAlign(bitsPerPixel / 8 * width, 32);
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(pixels, pixels, length, RDYYCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(pixels);
            return NULL;
        }
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, RDYYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, RDYYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_RDYYImageDecoderFrame *)frame {
    if (frame.dispose == RDYYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == RDYYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == RDYYImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_RDYYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == RDYYImageDisposePrevious) {
        if (frame.blend == RDYYImageBlendOver) {
...
@implementation RDYYImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"RDYYImageEncoder init error" reason:@"RDYYImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:RDYYImageTypeUnknown];
}

- (instancetype)initWithType:(RDYYImageType)type {
    if (type == RDYYImageTypeUnknown || type >= RDYYImageTypeOther) {
...
#if !RDYYIMAGE_WEBP_ENABLED
    if (type == RDYYImageTypeWebP) {
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/RDYYImage#installation", __FUNCTION__, __LINE__);
        return nil;
    }
#endif
    ...
    switch (type) {
        case RDYYImageTypeJPEG:
        case RDYYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case RDYYImageTypeTIFF:
        case RDYYImageTypeBMP:
        case RDYYImageTypeGIF:
        case RDYYImageTypeICO:
        case RDYYImageTypeICNS:
        case RDYYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, RDYYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, RDYYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == RDYYImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = RDYYCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return RDYYCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return RDYYCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == RDYYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = RDYYCGImageCreateEncodedData(decoded, RDYYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, RDYYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = RDYYCGImageCreateEncodedData(extendedImage, RDYYImageTypePNG, 1);
        if (!frameData) {
    ...
    png_header[0] = RDYY_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = RDYY_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == RDYY_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = yy_swap_endian_uint32(8); //length
            acTL[1] = RDYY_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            ...
            chunk_fcTL.dispose_op = RDYY_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = RDYY_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = yy_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = RDYY_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
        ...
        if (!insertAfter && insertBefore && chunk->fourcc != RDYY_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = RDYYCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, RDYYImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == RDYYImageTypePNG) return [self _encodeAPNG];
    if (_type == RDYYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(RDYYImageType)type quality:(CGFloat)quality {
    RDYYImageEncoder *encoder = [[RDYYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(RDYYImageDecoder *)decoder type:(RDYYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (RDYYImageCoder)

- (instancetype)yy_imageByDecoded {
    if (self.yy_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = RDYYCGImageCreateDecodedCopy(imageRef, YES);
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[RDYYSpriteSheetImage class]]) return YES;
...
/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for RDYYImage (PNG/JPEG/GIF/WebP)
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[RDYYImage class]]) {
        RDYYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == RDYYImageTypeGIF ||
                    image.animatedImageType == RDYYImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = RDYYCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
