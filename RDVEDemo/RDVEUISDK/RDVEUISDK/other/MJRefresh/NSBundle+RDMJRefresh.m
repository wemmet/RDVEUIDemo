...
#import "NSBundle+RDMJRefresh.h"
#import "RDMJRefreshComponent.h"
#import "RDMJRefreshConfig.h"

@implementation NSBundle (RDMJRefresh)
+ (instancetype)mj_refreshBundle
{
    static NSBundle *refreshBundle = nil;
    if (refreshBundle == nil) {
        // 这里不使用mainBundle是为了适配pod 1.x和0.x
        refreshBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[RDMJRefreshComponent class]] pathForResource:@"MJRefresh" ofType:@"bundle"]];
    }
...
+ (NSString *)mj_localizedStringForKey:(NSString *)key value:(NSString *)value
{
    ...
    if (bundle == nil) {
        NSString *language = RDMJRefreshConfig.defaultConfig.languageCode;
        // 如果配置中没有配置语言
        if (!language) {
