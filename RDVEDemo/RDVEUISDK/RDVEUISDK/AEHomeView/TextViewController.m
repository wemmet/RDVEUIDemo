...
@interface TextViewController ()<UITextViewDelegate, UIScrollViewDelegate>
{
    RDATMHud            *hud;
    UITextView          *textView;
...
- (void)viewDidLoad {
    ...
    selectedFontName = _textSource.fontName;
    selectedFontIndex = -1;
    fontSize = _textSource.fontSize;
    if (fontSize == 0.0) {
        fontSize = 20;
    }
    ...
    textView.text = _textSource.text;
    textView.font = [UIFont fontWithName:_textSource.fontName size:fontSize];
    textView.textColor = [UIColor whiteColor];
    textView.delegate = self;
    [self.view addSubview:textView];
    [textView becomeFirstResponder];
    
    placeHolderLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 1.5, textView.bounds.size.width - 20, 34)];
    if (_suggestionStr.length > 0) {
        placeHolderLabel.text = [NSString stringWithFormat:RDLocalizedString(@"文字%d(%@)", nil), _textIndex, _suggestionStr];
    }else {
        placeHolderLabel.text = [NSString stringWithFormat:RDLocalizedString(@"文字%d(最多%d行)", nil), _textIndex, _lineNum];
    }
    placeHolderLabel.textColor = [UIColor colorWithWhite:1.0 alpha:0.7];
    placeHolderLabel.font = [UIFont systemFontOfSize:fontSize];
    if (_textSource.text.length > 0) {
    ...
    __block int lines = 0;
    [_textSource.text enumerateSubstringsInRange:NSMakeRange(0, _textSource.text.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:
    ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        lines++;
    }];
    tipLbl.text = [NSString stringWithFormat:RDLocalizedString(@"字数：%d/%d 最多：%d行", nil), lines, _maxNum, _lineNum];
...
- (void)initFontScrollView {
    ...
    for (int k = 0; k<fontList.count; k++) {
        ...
        if (k == 0) {
            if ([[itemDic objectForKey:@"font"] isEqualToString:selectedFontName]) {
                selectedFontIndex = 0;
            }
        }else {
            NSString *fontfile;
            ...
            if(fontfile.length > 0){
                ...
                if ([fontName isEqualToString:selectedFontName]) {
                    selectedFontIndex = k;
                }
            }
        }
        ...
        NSString *path = [NSString stringWithFormat:@"%@/%@",kFontFolder,@"icon"];
        if(hasNew){
...
- (void)getFontList {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        NSString *fontUrl = @"";
        if(hasNew){
            fontUrl = ((RDNavigationViewController *)strongSelf.navigationController).editConfiguration.fontResourceURL;
            fontListDic = [RDHelpClass getNetworkMaterialWithType:kFontType
                                                           appkey:((RDNavigationViewController *)strongSelf.navigationController).appKey
                                                          urlPath:fontUrl];
        }else{
        ...
        [fontdic setObject:strongSelf.templateFontName forKey:@"font"];
        
        BOOL resultInteger = hasNew ? [fontListDic[@"code"] intValue] == 0 : [fontListDic[@"code"] intValue] == 200;
        if (!resultInteger){
            fontListDic = nil;
            if (!strongSelf->fontList) {//20161108 bug4320
                strongSelf->fontList = [[NSMutableArray alloc] initWithObjects:fontdic, nil];
            }
        }else{
            if([fontListDic[@"data"] isKindOfClass:[NSMutableArray class]]){
                strongSelf->fontList = [fontListDic[@"data"] mutableCopy];
                [strongSelf->fontList insertObject:fontdic atIndex:0];
                
                BOOL suc = [strongSelf->fontList writeToFile:kFontPlistPath atomically:YES];
                if(!suc){
                    NSLog(@"写文件失败");
                }
                if (!hasNew && (!strongSelf->fontIconList || (strongSelf->fontIconList && [[[fontListDic objectForKey:@"icon"] objectForKey:@"timeunix"] longValue] > [[strongSelf->fontIconList objectForKey:@"timeunix"] longValue]))) {
                    NSFileManager *manager = [[NSFileManager alloc] init];
                    NSError *error;
                    NSString *path = [NSString stringWithFormat:@"%@/%@",kFontFolder,[strongSelf->fontIconList objectForKey:@"name"]];
                    if([manager fileExistsAtPath:path]){
                        [manager removeItemAtPath:path error:&error];
                    }
                    create = NO;
                    strongSelf->fontIconList = [fontListDic objectForKey:@"icon"];
                    suc = [strongSelf->fontIconList writeToFile:kFontIconPlistPath atomically:YES];
                    if(!suc){
                        NSLog(@"写文件失败");
                    }
                    if (!create) {
                        [strongSelf DownloadThumbnailFile:[strongSelf->fontIconList objectForKey:@"caption"] andUnzipToPath:@"SubtitleEffect/Font" andCellcaptionTypeCount:1];
                    }
                }else if(!create){
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [strongSelf initFontScrollView];
                    });
                }
            }
        }
...
- (void)buttonAction:(UIButton *)sender {
    ...
    if (sender.tag == 2) {
        if (textView.text.length > 0) {
            _textSource.text = [textView.text stringByReplacingOccurrencesOfString:@"\\n" withString:@"\n"];
        }
        _textSource.fontName = selectedFontName;
        if (_delegate && [_delegate respondsToSelector:@selector(editTextFinished:)]) {
            [_delegate editTextFinished:selectedFontIndex];
        }
...
- (void)setFont:(NSInteger)index{
    ...
    if(index==0){
        selectedFontName = _templateFontName;
    ...
    selectedFontName = [RDHelpClass customFontWithPath:path fontName:netFontName];
...
- (void)setFontSize:(NSString *)string{
    ...
    if(size<1){
        if(selectedFontName){
            textView.font = [UIFont fontWithName:selectedFontName size:fontSize*size];
        }else{
            textView.font = [UIFont systemFontOfSize:fontSize*size];
        }
    }else{
        if(selectedFontName){
            textView.font = [UIFont fontWithName:selectedFontName size:fontSize];
        }else{
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    [comcatstr enumerateSubstringsInRange:NSMakeRange(0, comcatstr.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:
    ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        lines++;
    }];
    NSInteger caninputlen = _maxNum - lines;
    ...
    else
    {
        ...
        NSInteger len = lines + caninputlen;
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    __block NSInteger existTextNum = 0;
    [nsTextContent enumerateSubstringsInRange:NSMakeRange(0, nsTextContent.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:
    ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        existTextNum++;
    }];
    ...
    tipLbl.text = [NSString stringWithFormat:RDLocalizedString(@"字数：%d/%d 最多：%d行", nil), existTextNum, _maxNum, _lineNum];
}
