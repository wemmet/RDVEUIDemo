...
#import "RD_JSONModelClassProperty.h"
...
static RD_JSONValueTransformer* valueTransformer = nil;
static Class RD_JSONModelClass = NULL;

#pragma mark - model cache
static RD_JSONKeyMapper* globalKeyMapper = nil;

#pragma mark - JSONModel implementation
@implementation RD_JSONModel
{
    ...
    dispatch_once(&once, ^{
        ...
        @autoreleasepool {
            allowedJSONTypes = @[
                ...
                [NSMutableString class], [NSMutableArray class], [NSMutableDictionary class] , [NSData class]//mutable JSON classes
            ];
            ...
            valueTransformer = [[RD_JSONValueTransformer alloc] init];
            ...
            RD_JSONModelClass = NSClassFromString(NSStringFromClass(self));
        }
    });
}
...
-(instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err
{
    //check for nil input
    if (!data) {
        if (err) *err = [RD_JSONModelError errorInputIsNil];
        return nil;
    }
    //read the json
    ...
    if (initError) {
        if (err) *err = [RD_JSONModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithString:(NSString*)string error:(RD_JSONModelError**)err
{
    RD_JSONModelError* initError = nil;
...
-(id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(RD_JSONModelError**)err
{
    //check for nil input
    if (!string) {
        if (err) *err = [RD_JSONModelError errorInputIsNil];
...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [RD_JSONModelError errorInvalidDataWithMessage:@"Attempt to initialize JSONModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [RD_JSONModelError errorModelIsInvalid];
        return nil;
    }
...
-(RD_JSONKeyMapper*)__keyMapper
{
    //get the model key mapper
    return objc_getAssociatedObject(self.class, &kMapperObjectKey);
}

-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(RD_JSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (RD_JSONModelClassProperty* property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        if (err) *err = [RD_JSONModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(RD_JSONKeyMapper*)keyMapper
{
    if (keyMapper) {
        //custom mapper
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(RD_JSONKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (RD_JSONModelClassProperty* property in [self __properties__]) {
        ...
        if (isRDNull(jsonValue)) {
            ...
            if (err) {
                ...
                RD_JSONModelError* dataErr = [RD_JSONModelError errorInvalidDataWithMessage:msg];
        ...
        if (property) {
            ...
            if ([self __isJSONModelSubClass:property.type]) {
                ...
                if (
                    (![jsonValue isKindOfClass:property.type] && !isRDNull(jsonValue))
                    ||
                    ...
                    Class sourceClass = [RD_JSONValueTransformer classByResolvingClusterClasses:[jsonValue class]];
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            RD_JSONModelError* dataErr = [RD_JSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
#ifdef UNIT_TESTING
    return [@"RD_JSONModel" isEqualToString: NSStringFromClass([class superclass])];
#else
    return [class isSubclassOfClass:RD_JSONModelClass];
#endif
}
...
-(NSMutableSet*)__requiredPropertyNames
{
    ...
    if (!classRequiredPropertyNames) {
        ...
        [[self __properties__] enumerateObjectsUsingBlock:^(RD_JSONModelClassProperty* p, NSUInteger idx, BOOL *stop) {
...
-(void)__inspectProperties
{
    ...
    while (class != [RD_JSONModel class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            RD_JSONModelClassProperty* p = [[RD_JSONModelClassProperty alloc] init];
...
#pragma GCC diagnostic pop
            ...
            if (p)
            {
                ...
                {
                    NSString *class = NSStringFromClass([RD_JSONValueTransformer classByResolvingClusterClasses:type]);
...
-(id)__transform:(id)value forProperty:(RD_JSONModelClassProperty*)property error:(NSError**)err
{
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
            ...
            if(![[value class] isSubclassOfClass:[NSArray class]])
            {
                if(err != nil)
                {
                    ...
                    RD_JSONModelError* typeErr = [RD_JSONModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            //one shot conversion
            RD_JSONModelError* arrayErr = nil;
...
-(id)__reverseTransform:(id)value forProperty:(RD_JSONModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
    ...
    if ([self __isJSONModelSubClass:protocolClass]) {
        ...
        if (property.type == [NSArray class] || property.type == [NSMutableArray class]) {
            ...
            for (int i = 0;i<[((NSArray*)value) count] ; i++) {
                NSObject<AbstractJSONModelProtocol>* model = ((NSArray*)value)[i];
...
- (BOOL)__customSetValue:(id <NSObject>)value forProperty:(RD_JSONModelClassProperty *)property
{
    NSString *class = NSStringFromClass([RD_JSONValueTransformer classByResolvingClusterClasses:[value class]]);
...
- (BOOL)__customGetValue:(id *)value forProperty:(RD_JSONModelClassProperty *)property
{
...
-(NSDictionary*)toDictionaryWithKeys:(NSArray*)propertyNames
{
    ...
    for (RD_JSONModelClassProperty* p in properties) {
        ...
        if ([value isKindOfClass:RD_JSONModelClass]) {

            //recurse models
            value = [(RD_JSONModel*)value toDictionary];
            [tempDictionary setValue:value forKeyPath: keyPath];
...
+ (NSMutableDictionary *)dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError **)err
{
    ...
    for (NSString *key in dictionary.allKeys)
    {
        ...
        {
            if (err) {
                *err = [RD_JSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
...
{
    //bail early
    if (isRDNull(dictionary)) return nil;
...
-(NSString*)description
{
    ...
    for (RD_JSONModelClassProperty *p in [self __properties__]) {
...
+(RD_JSONKeyMapper*)keyMapper
{
    return nil;
}

+(void)setGlobalKeyMapper:(RD_JSONKeyMapper*)globalKeyMapperParam
{
