...
@implementation RD_JSONValueTransformer
...
+(Class)classByResolvingClusterClasses:(Class)sourceClass
{
    ...
    if ([sourceClass isSubclassOfClass:[NSData class]]) {
        return [NSData class];
    }
...
{
    if (isRDNull(number)) return [NSNumber numberWithBool:NO];
