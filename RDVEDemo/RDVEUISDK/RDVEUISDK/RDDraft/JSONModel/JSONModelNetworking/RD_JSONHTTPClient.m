...
typedef void (^RequestResultBlock)(NSData *data, RD_JSONModelError *error);

#pragma mark - constants
NSString* const kRDHTTPMethodGET = @"GET";
NSString* const kRDHTTPMethodPOST = @"POST";

NSString* const kRDContentTypeAutomatic    = @"jsonmodel/automatic";
NSString* const kRDContentTypeJSON         = @"application/json";
NSString* const kRDContentTypeWWWEncoded   = @"application/x-www-form-urlencoded";
...
@implementation RD_JSONHTTPClient
...
+(void)initialize
{
    ...
    dispatch_once(&once, ^{
        ...
        requestContentType = kRDContentTypeAutomatic;
    });
}
...
+(NSString*)contentTypeForRequestString:(NSString*)requestString
{
    ...
    if (requestString.length>0 && [contentType isEqualToString:kRDContentTypeAutomatic]) {
        ...
        if ([firstAndLastChar isEqualToString:@"{}"] || [firstAndLastChar isEqualToString:@"[]"]) {
            ...
            contentType = kRDContentTypeJSON;
        } else {
            //fallback to www form encoded params
            contentType = kRDContentTypeWWWEncoded;
        }
    }
...
+(void)requestDataFromURL:(NSURL*)url method:(NSString*)method requestBody:(NSData*)bodyData headers:(NSDictionary*)headers handler:(RequestResultBlock)handler
{
    ...
    if ([requestContentType isEqualToString:kRDContentTypeAutomatic]) {
    ...
    void (^completionHandler)(NSData *, NSURLResponse *, NSError *) = ^(NSData *data, NSURLResponse *origResponse, NSError *origError) {
        ...
        if (origError) {
            error = [RD_JSONModelError errorWithDomain:origError.domain code:origError.code userInfo:origError.userInfo];
        }
        ...
        if (!error && (response.statusCode >= 300 || response.statusCode < 200)) {
            error = [RD_JSONModelError errorBadResponse];
        }
...
+(void)requestDataFromURL:(NSURL*)url method:(NSString*)method params:(NSDictionary*)params headers:(NSDictionary*)headers handler:(RequestResultBlock)handler
{
    ...
    if ([method isEqualToString:kRDHTTPMethodGET] && params) {
    ...
    [self requestDataFromURL: url
                      method: method
                 requestBody: [method isEqualToString:kRDHTTPMethodPOST]?[paramsString dataUsingEncoding:NSUTF8StringEncoding]:nil
                     headers: headers
                       handler:handler];
}

#pragma mark - Async network request
+(void)JSONFromURLWithString:(NSString*)urlString method:(NSString*)method params:(NSDictionary*)params orBodyString:(NSString*)bodyString completion:(RD_JSONObjectBlock)completeBlock
{
...
+(void)JSONFromURLWithString:(NSString *)urlString method:(NSString *)method params:(NSDictionary *)params orBodyString:(NSString *)bodyString headers:(NSDictionary *)headers completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString
                         method:method
                         params:params
...
+(void)JSONFromURLWithString:(NSString*)urlString method:(NSString*)method params:(NSDictionary *)params orBodyData:(NSData*)bodyData headers:(NSDictionary*)headers completion:(RD_JSONObjectBlock)completeBlock
{
    RequestResultBlock handler = ^(NSData *responseData, RD_JSONModelError *error) {
        id jsonObject = nil;
...
+(void)getJSONFromURLWithString:(NSString*)urlString completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString method:kRDHTTPMethodGET
                         params:nil
                   orBodyString:nil completion:^(id json, RD_JSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}

+(void)getJSONFromURLWithString:(NSString*)urlString params:(NSDictionary*)params completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString method:kRDHTTPMethodGET
                         params:params
                   orBodyString:nil completion:^(id json, RD_JSONModelError* e) {
...
+(void)postJSONFromURLWithString:(NSString*)urlString params:(NSDictionary*)params completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString method:kRDHTTPMethodPOST
                         params:params
                   orBodyString:nil completion:^(id json, RD_JSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
...
+(void)postJSONFromURLWithString:(NSString*)urlString bodyString:(NSString*)bodyString completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString method:kRDHTTPMethodPOST
                         params:nil
                   orBodyString:bodyString completion:^(id json, RD_JSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}

+(void)postJSONFromURLWithString:(NSString*)urlString bodyData:(NSData*)bodyData completion:(RD_JSONObjectBlock)completeBlock
{
    [self JSONFromURLWithString:urlString method:kRDHTTPMethodPOST
                         params:nil
                   orBodyString:[[NSString alloc] initWithData:bodyData encoding:defaultTextEncoding]
                                 completion:^(id json, RD_JSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}
