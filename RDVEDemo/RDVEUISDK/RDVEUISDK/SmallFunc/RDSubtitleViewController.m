...
@interface RDSubtitleViewController ()<RDVECoreDelegate, UIAlertViewDelegate,RDAddEffectsByTimelineDelegate>
{
    UIView                  * titleView;
    UIView                  * toolBarView;
    RDVECore                * rdPlayer;
...
@implementation RDSubtitleViewController
...
- (void)viewDidLoad {
    ...
    _addEffectsByTimeline.currentEffect = RDAdvanceEditType_Subtitle;
...
- (void)initTitleView {
    ...
    label.text = RDLocalizedString(@"字幕", nil);
...
- (void)initToolBarView{
    ...
    [cancelBtn addTarget:self action:@selector(cancelEditSubtitle) forControlEvents:UIControlEventTouchUpInside];
    ...
    [finishAddSubtitleBtn addTarget:self action:@selector(finishEditSubtitle) forControlEvents:UIControlEventTouchUpInside];
...
- (void)initPlayer {
    ...
    rdPlayer.frame = self.playerView.bounds;
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        if (view.file.caption) {
            ...
            if(isNetSource){
                ...
                NSString *iconPath = [NSString stringWithFormat:@"%@/%@.png",kSubtitleIconPath,view.file.caption.imageName];
