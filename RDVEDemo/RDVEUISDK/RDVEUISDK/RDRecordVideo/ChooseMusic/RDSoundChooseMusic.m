...
@interface RDSoundDChooseMusic ()<RDCloudMusicItemViewControllerDelegate,UITextViewDelegate,UIAlertViewDelegate>
{
    UIButton *_leftBtn;
...
@implementation RDSoundDChooseMusic
...
- (void)viewDidLoad {
    ...
    _customCloudMusic = [_navigationController.editConfiguration.cloudMusicResourceURL isEqualToString:@"http://dianbook.17rd.com/api/shortvideo/getcloudmusic"] ? NO : YES;
    NSError *error;
...
//    [self.navigationController.view addSubview:_hud.view];
    ...
    _titleView.tintColor = [UIColor clearColor];
    _titleView.normalColor = [UIColor whiteColor];
    _titleView.selectedColor = Main_Color;
    ...
    if([CloudMusicResourceURL isEqual:_cloudMusicResourceURL]){
           _titleView.frame = CGRectMake(35, 0, self.frame.size.width - 35, 55);
           _contentView.frame = CGRectMake(0, 55, self.frame.size.width, self.frame.size.height - 55);
       }else{
           _titleView.frame = CGRectMake(35, 0, self.frame.size.width - 35, 55);
           CGRect rect =  CGRectMake(0, 55, self.frame.size.width, self.frame.size.height - 55);
    ...
    [_leftBtn setFrame:CGRectMake(0, (55-35)/2.0, 35, 35)];
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, _titleView.bounds.size.height - 1, self.frame.size.width, 1)];
    view.backgroundColor = UIColorFromRGB(0x888888);
    [self addSubview:view];
    
    EditConfiguration *editConfig = _navigationController.editConfiguration;
    ...
    if(_cloudMusicList){
        if([lexiu currentReachabilityStatus] != RDNotReachable){
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                if(!_customCloudMusic){
                    ...
                    params = [[NSMutableDictionary alloc] initWithObjectsAndKeys:@"cloud_music_type",@"type",_navigationController.appKey,@"appkey", nil];
        ...
        if([lexiu currentReachabilityStatus] != RDNotReachable){
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if( !_isNOSound )
                {
                    NSDictionary *dic1 = [RDHelpClass getNetworkMaterialWithType:@"audio" appkey:_navigationController.appKey urlPath:_soundMusicTypeResourceURL];
                ...
                else
                {
                    if( [_cloudMusicResourceURL rangeOfString:@"http://d.56show.com/filemanage2/public/filemanage/file"].location == NSNotFound )
                    {
                        ...
                        if(!_customCloudMusic){
                            ...
                            dic = [RDHelpClass getNetworkMaterialWithType:@"cloud_music_type"
                                                                   appkey:_navigationController.appKey
                    ...
                    {
                        NSDictionary *dic1 = [RDHelpClass getNetworkMaterialWithType:@"cloud_music" appkey:_navigationController.appKey urlPath:_soundMusicTypeResourceURL];
                        NSMutableArray *soundArray = [NSMutableArray alloc];
                        if( [[dic1 objectForKey:@"code"] integerValue] == 0 )
                        {
                            if([[dic1 objectForKey:@"data"] isKindOfClass:[NSArray class]]){
...
- (void)refreshContentView {
    ...
    for (int i = 0; i<titles.count; i ++) {
        ...
        vc.fheight = _contentView.frame.size.height;
        [_vcs addObject:vc];
    }
    if (_vcs.count > 0) {
        [_contentView reloadViewWithChildVcs:_vcs parentVC:_viewController];
    }
}
