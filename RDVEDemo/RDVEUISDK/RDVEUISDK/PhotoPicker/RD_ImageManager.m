...
//
//  Created by com.rdtd on 16/1/4.
//  Copyright © 2016年 com.rdtd. All rights reserved.
//

#import "RD_ImageManager.h"
#import "RD_AssetModel.h"

@interface RD_ImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation RD_ImageManager

CGSize RDAssetGridThumbnailSize;
CGFloat RDTZScreenWidth;
CGFloat RDTZScreenScale;

static RD_ImageManager *manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (RDTZScreenWidth - 2 * margin - 4) / columnNumber - margin;
    RDAssetGridThumbnailSize = CGSizeMake(itemWH * RDTZScreenScale, itemWH * RDTZScreenScale);
}

- (void)configTZScreenWidth {
    RDTZScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    RDTZScreenScale = 2.0;
    if (RDTZScreenWidth > 700) {
        RDTZScreenScale = 1.5;
    }
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(RDTZAlbumModel *model))completion {
    __block RDTZAlbumModel *model;
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<RDTZAlbumModel *> *))completion{
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<RD_AssetModel *> *))completion {
    return [self getAssetsFromFetchResult:result allowPickingVideo:NO allowPickingImage:NO completion:completion];
}

- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<RD_AssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        RD_AssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(RD_AssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (RD_AssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    if (!canSelect) return nil;
    
    RD_AssetModel *model;
    RD_AssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == RD_AssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == RD_AssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == RD_AssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == RD_AssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [RD_AssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (RD_AssetModelMediaType)getAssetType:(PHAsset *)asset {
    RD_AssetModelMediaType type = RD_AssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = RD_AssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = RD_AssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = RD_AssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = RD_AssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        RD_AssetModel *model = photos[i];
        
        if (model.type == RD_AssetModelMediaTypeVideo) {
            ...
            if (model.type == RD_AssetModelMediaTypePhotoGif) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
                if (model.type != RD_AssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = RDTZScreenWidth;
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < RDTZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = RDAssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * RDTZScreenScale * 1.5;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (void)getPostImageWithAlbumModel:(RDTZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    id asset = [model.result lastObject];
    if (!self.sortAscendingByModificationDate) {
        asset = [model.result firstObject];
    }
    [[RD_ImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (RDTZAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    RDTZAlbumModel *model = [[RDTZAlbumModel alloc] init];
...
- (RD_AssetModel *)createModelWithAsset:(PHAsset *)asset {
    RD_AssetModelMediaType type = [[RD_ImageManager manager] getAssetType:asset];
    NSString *timeLength = type == RD_AssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[RD_ImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    RD_AssetModel *model = [RD_AssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//    return [NSNumber numberWithBool:![RD_ImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
