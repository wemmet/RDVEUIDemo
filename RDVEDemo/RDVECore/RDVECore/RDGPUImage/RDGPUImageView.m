...
@interface RDGPUImageView () 
{
    RDGPUImageFramebuffer *inputFramebufferForDisplay;
    GLuint displayRenderbuffer, displayFramebuffer;
    
    RDGLProgram *displayProgram;
...
- (void)commonInit;
{
    ...
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        displayProgram = [[RDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kRDGPUImageVertexShaderString fragmentShaderString:kRDGPUImagePassthroughFragmentShaderString];
        ...
        [RDGPUImageContext setActiveShaderProgram:displayProgram];
        glEnableVertexAttribArray(displayPositionAttribute);
        glEnableVertexAttribArray(displayTextureCoordinateAttribute);
        
        [self setBackgroundColor:[UIColor clearColor]];
//        [self setBackgroundColorRed:0.0 green:0.0 blue:0.0 alpha:1.0];
        _fillMode = kRDGPUImageFillModePreserveAspectRatio;
...
//    [[[RDGPUImageContext sharedImageProcessingContext] context] renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer*)self.layer];
    if (![[[RDGPUImageContext sharedImageProcessingContext] context] renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer*)self.layer]) {
        return;
    }
...
{
    glBindRenderbuffer(GL_RENDERBUFFER, displayRenderbuffer);
    [[RDGPUImageContext sharedImageProcessingContext] presentBufferForDisplay];
}
...
- (void)setFrame:(CGRect)frame{
    ...
    _mybounds = CGRectMake(0, 0, frame.size.width, frame.size.height);
}
- (void)recalculateViewGeometry;
{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        CGSize currentViewSize = _mybounds.size;
        
        //    CGFloat imageAspectRatio = inputImageSize.width / inputImageSize.height;
        //    CGFloat viewAspectRatio = currentViewSize.width / currentViewSize.height;
        
        CGRect insetRect = AVMakeRectWithAspectRatioInsideRect(inputImageSize, _mybounds);
        
        switch(_fillMode)
        {
            case kRDGPUImageFillModeStretch:
            {
                widthScaling = 1.0;
                heightScaling = 1.0;
            }; break;
            case kRDGPUImageFillModePreserveAspectRatio:
            {
                widthScaling = insetRect.size.width / currentViewSize.width;
                heightScaling = insetRect.size.height / currentViewSize.height;
            }; break;
            case kRDGPUImageFillModePreserveAspectRatioAndFill:
            {
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        glVertexAttribPointer(displayTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [RDGPUImageView textureCoordinatesForRotation:inputRotation]);
...
- (void)setFillMode:(RDGPUImageFillModeType)newValue;
{
    _fillMode = newValue;
    [self recalculateViewGeometry];
}
