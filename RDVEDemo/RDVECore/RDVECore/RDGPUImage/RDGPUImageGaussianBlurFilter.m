...
@implementation RDGPUImageGaussianBlurFilter
...
+ (NSString *)vertexShaderForStandardBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    if (blurRadius < 1)
    {
        return kRDGPUImageVertexShaderString;
    }
    
//    NSLog(@"Max varyings: %d", [RDGPUImageContext maximumVaryingVectorsForThisDevice]);
...
+ (NSString *)fragmentShaderForStandardBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    if (blurRadius < 1)
    {
        return kRDGPUImagePassthroughFragmentShaderString;
    }
...
- (void)renderToTextureWithVertices:(const GLfloat *)vertices textureCoordinates:(const GLfloat *)textureCoordinates;
{
    ...
    {
        [super renderToTextureWithVertices:vertices textureCoordinates:[[self class] textureCoordinatesForRotation:kRDGPUImageNoRotation]];
    }
}
...
- (void)switchToVertexShader:(NSString *)newVertexShader fragmentShader:(NSString *)newFragmentShader;
{
    rdRunSynchronouslyOnVideoProcessingQueue(^{
        ...
        filterProgram = [[RDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:newVertexShader fragmentShaderString:newFragmentShader];
        ...
        secondFilterProgram = [[RDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:newVertexShader fragmentShaderString:newFragmentShader];
