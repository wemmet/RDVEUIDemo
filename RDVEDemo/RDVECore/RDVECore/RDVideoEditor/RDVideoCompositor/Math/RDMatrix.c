...
void RDMatrixRotate(RDMatrix4 * result, float angle, float x, float y, float z)
{
	...
	if ( mag > 0.0f )
	{
		...
		RDMatrixMultiply( result, &rotMat, result );
	}
}

// result[x][y] = a[x][0]*b[0][y]+a[x][1]*b[1][y]+a[x][2]*b[2][y]+a[x][3]*b[3][y];
void RDMatrixMultiply(RDMatrix4 * result, const RDMatrix4 *a, const RDMatrix4 *b)
{
	RDMatrix4 tmp;
	int i;
	...
	memcpy(result, &tmp, sizeof(RDMatrix4));
}
...
{
	memcpy(target, src, sizeof(RDMatrix4));
}
...
void RDMatrix4ToMatrix3(RDMatrix3 * result, const RDMatrix4 * src)
{
...
{
	memset(result, 0x0, sizeof(RDMatrix4));
...
void RDFrustum(RDMatrix4 * result, float left, float right, float bottom, float top, float nearZ, float farZ)
{
	...
	RDMatrixMultiply(result, &frust, result);
}
...
void RDPerspective(RDMatrix4 * result, float fovy, float aspect, float nearZ, float farZ)
{
	...
	RDFrustum(result, -frustumW, frustumW, -frustumH, frustumH, nearZ, farZ);
}
...
void RDOrtho(RDMatrix4 * result, float left, float right, float bottom, float top, float nearZ, float farZ)
{
	...
	RDMatrixMultiply(result, &ortho, result);
}

void RDLookAt(RDMatrix4 * result, const RDVec3 * eye, const RDVec3 * target, const RDVec3 * up)
{
	RDVec3 side, up2, forward ;
	//RDVec4 eyePrime;
	RDMatrix4 transMat;

	RDVectorSubtract(&forward, target, eye);
	RDVectorNormalize(&forward);

	RDCrossProduct(&side, up, &forward);
	RDVectorNormalize(&side );

	RDCrossProduct(&up2, &side, &forward);
	RDVectorNormalize(&up2);

	RDMatrixLoadIdentity(result);
	result->m[0][0] = side.x;
	result->m[0][1] = side.y;
	result->m[0][2] = side.z;
	result->m[1][0] = up2.x;
	...
	RDMatrixLoadIdentity(&transMat);
	RDMatrixTranslate(&transMat, -eye->x, -eye->y, -eye->z);

	RDMatrixMultiply(result, result, &transMat);
	...
	//RDMatrixMultiplyVector(&eyePrime, result, &eyePrime);
	//RDMatrixTranspose(result, result);
