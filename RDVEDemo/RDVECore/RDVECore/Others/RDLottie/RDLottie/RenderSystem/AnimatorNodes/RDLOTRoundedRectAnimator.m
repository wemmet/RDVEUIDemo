...
@implementation RDLOTRoundedRectAnimator {
  RDLOTPointInterpolator *_centerInterpolator;
  RDLOTPointInterpolator *_sizeInterpolator;
  RDLOTNumberInterpolator *_cornerRadiusInterpolator;
  BOOL _reversed;
}
...
- (instancetype _Nonnull)initWithInputNode:(RDLOTAnimatorNode *_Nullable)inputNode
  ...
  if (self) {
    _centerInterpolator = [[RDLOTPointInterpolator alloc] initWithKeyframes:shapeRectangle.position.keyframes];
    _sizeInterpolator = [[RDLOTPointInterpolator alloc] initWithKeyframes:shapeRectangle.size.keyframes];
    _cornerRadiusInterpolator = [[RDLOTNumberInterpolator alloc] initWithKeyframes:shapeRectangle.cornerRadius.keyframes];
...
- (void)addCorner:(CGPoint)cornerPoint withRadius:(CGFloat)radius toPath:(RDLOTBezierPath *)path clockwise:(BOOL)clockwise {
  ...
  if (cornerPoint.y == currentPoint.y) {
    ...
    if (cornerPoint.x < currentPoint.x) {
      ...
      [path RDLOT_addLineToPoint:corner];
      if (radius) {
        ...
        CGPoint cp2 = (clockwise ?
                       ...
                       CGPointMake(curvePoint.x, curvePoint.y - (radius * ellipseControlPointPercentage)));
        [path RDLOT_addCurveToPoint:curvePoint controlPoint1:cp1 controlPoint2:cp2];
      }
    } else {
      // Moving east
      CGPoint corner = CGPointMake(cornerPoint.x - radius, currentPoint.y);
...
- (void)performLocalUpdate {
  ...
  RDLOTBezierPath *path1 = [[RDLOTBezierPath alloc] init];
  ...
  [path1 RDLOT_moveToPoint:startPoint];
  if (clockWise) {
