...
#import "RDLOTRadialGradientLayer.h"

@implementation RDLOTGradientFillRender {
  BOOL _evenOddFillRule;
  CALayer *centerPoint_DEBUG;
  
  CAShapeLayer *_maskShape;
  RDLOTRadialGradientLayer *_gradientOpacityLayer;
  RDLOTRadialGradientLayer *_gradientLayer;
  NSInteger _numberOfPositions;
  
  CGPoint _startPoint;
  CGPoint _endPoint;
  
  RDLOTArrayInterpolator *_gradientInterpolator;
  RDLOTPointInterpolator *_startPointInterpolator;
  RDLOTPointInterpolator *_endPointInterpolator;
  RDLOTNumberInterpolator *_opacityInterpolator;
}

- (instancetype)initWithInputNode:(RDLOTAnimatorNode *)inputNode
                          shapeGradientFill:(RDLOTShapeGradientFill *)fill {
  self = [super initWithInputNode:inputNode keyName:fill.keyname];
  if (self) {
    _gradientInterpolator = [[RDLOTArrayInterpolator alloc] initWithKeyframes:fill.gradient.keyframes];
    _startPointInterpolator = [[RDLOTPointInterpolator alloc] initWithKeyframes:fill.startPoint.keyframes];
    _endPointInterpolator = [[RDLOTPointInterpolator alloc] initWithKeyframes:fill.endPoint.keyframes];
    _opacityInterpolator = [[RDLOTNumberInterpolator alloc] initWithKeyframes:fill.opacity.keyframes];
    ...
    _gradientOpacityLayer = [RDLOTRadialGradientLayer new];
    _gradientOpacityLayer.isRadial = (fill.type == RDLOTGradientTypeRadial);
    ...
    _gradientLayer = [RDLOTRadialGradientLayer new];
    _gradientLayer.isRadial = (fill.type == RDLOTGradientTypeRadial);
