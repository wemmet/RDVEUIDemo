...
- (CGPoint)lerpWithStartPoint:(CGPoint)start EndPoint:(CGPoint)end CurrentValue:(float)t
{
...
- (CGPoint)lerpSelfWithStartPoint:(CGPoint)start EndPoint:(CGPoint)end CurrentValue:(float)t
{
...
- (CGPoint)fromBezier:(CGPoint*)bezierPoint CurrentValue:(float)t
{
    CGPoint a = bezierPoint[0];
    CGPoint b = bezierPoint[1];
    CGPoint c = bezierPoint[2];
    CGPoint d = bezierPoint[3];
    
    CGPoint ab = [self lerpWithStartPoint:a EndPoint:b CurrentValue:t];//lerp(a, b, t);
    CGPoint bc = [self lerpWithStartPoint:b EndPoint:c CurrentValue:t];//lerp(b, c, t);
    CGPoint cd = [self lerpWithStartPoint:c EndPoint:d CurrentValue:t];//lerp(c, d, t);
    
    ab = [self lerpSelfWithStartPoint:ab EndPoint:bc CurrentValue:t];//lerpSelf(ab, bc, t);
    bc = [self lerpSelfWithStartPoint:bc EndPoint:cd CurrentValue:t];//lerpSelf(bc, cd, t);
    
    return [self lerpSelfWithStartPoint:ab EndPoint:bc CurrentValue:t];//lerpSelf(ab, bc, t);
}
@end
