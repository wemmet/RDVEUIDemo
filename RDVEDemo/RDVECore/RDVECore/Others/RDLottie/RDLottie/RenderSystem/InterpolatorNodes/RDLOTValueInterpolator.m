...
@interface RDLOTValueInterpolator ()

@property (nonatomic, strong) NSArray<RDLOTKeyframe *> *keyframes;

@end

@implementation RDLOTValueInterpolator
...
- (void)updateKeyframeSpanForFrame:(NSNumber *)frame {
  if (self.leadingKeyframe == nil &&
    ...
    RDLOTKeyframe *first = _keyframes.firstObject;
  ...
  if (self.trailingKeyframe && frame.floatValue >= self.trailingKeyframe.keyframeTime.floatValue) {
    ...
    RDLOTKeyframe *testLeading = self.trailingKeyframe;
    RDLOTKeyframe *testTrailing = nil;
    ...
    RDLOTKeyframe *testLeading = nil;
    RDLOTKeyframe *testTrailing = self.leadingKeyframe;
...
- (CGFloat)progressForFrame:(NSNumber *)frame {
  ...
  CGFloat progession = RDLOT_RemapValue(frame.floatValue, self.leadingKeyframe.keyframeTime.floatValue, self.trailingKeyframe.keyframeTime.floatValue, 0, 1);
  ...
  if ((self.leadingKeyframe.outTangent.x != self.leadingKeyframe.outTangent.y ||
      ...
      (!RDLOT_CGPointIsZero(self.leadingKeyframe.outTangent) &&
       !RDLOT_CGPointIsZero(self.trailingKeyframe.inTangent))) {
    // Bezier Time Curve
    progession = RDLOT_CubicBezeirInterpolate(CGPointMake(0, 0), self.leadingKeyframe.outTangent, self.trailingKeyframe.inTangent, CGPointMake(1, 1), progession);
  }
