...
// Static cache of looked up color names. Used with +RDLOT_colorWithName:
...
- (NSArray *)RDLOT_arrayFromRGBAComponents {
	NSAssert(self.canProvideRGBComponents, @"Must be an RGB color to use -RDLOT_arrayFromRGBAComponents");
  
	CGFloat r,g,b,a;
	if (![self RDLOT_red:&r green:&g blue:&b alpha:&a]) return nil;
...
- (BOOL)RDLOT_red:(CGFloat *)red green:(CGFloat *)green blue:(CGFloat *)blue alpha:(CGFloat *)alpha {
...
- (UInt32)rgbHex {
	...
	CGFloat r,g,b,a;
	if (![self RDLOT_red:&r green:&g blue:&b alpha:&a]) return 0;
...
- (UIColor *)RDLOT_colorByMultiplyingBy:(CGFloat)f {
	return [self RDLOT_colorByMultiplyingByRed:f green:f blue:f alpha:1.0f];
}

- (UIColor *)RDLOT_colorByAdding:(CGFloat)f {
	return [self RDLOT_colorByMultiplyingByRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)RDLOT_colorByLighteningTo:(CGFloat)f {
	return [self RDLOT_colorByLighteningToRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)RDLOT_colorByDarkeningTo:(CGFloat)f {
	return [self RDLOT_colorByDarkeningToRed:f green:f blue:f alpha:1.0f];
}
...
- (UIColor *)RDLOT_colorByMultiplyingByColor:(UIColor *)color {
	...
	return [self RDLOT_colorByMultiplyingByRed:r green:g blue:b alpha:1.0f];
}
...
- (UIColor *)RDLOT_colorByAddingColor:(UIColor *)color {
	...
	return [self RDLOT_colorByAddingRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)RDLOT_colorByLighteningToColor:(UIColor *)color {
	...
	return [self RDLOT_colorByLighteningToRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)RDLOT_colorByDarkeningToColor:(UIColor *)color {
	...
	return [self RDLOT_colorByDarkeningToRed:r green:g blue:b alpha:1.0f];
}

#pragma mark String utilities

- (NSString *)RDLOT_stringFromColor {
	NSAssert(self.canProvideRGBComponents, @"Must be an RGB color to use -RDLOT_stringFromColor");
	NSString *result;
	switch (self.colorSpaceModel) {
		case kCGColorSpaceModelRGB:
...
// Returns a UIColor by scanning the string for a hex number and passing that to +[UIColor RDLOT_colorWithRGBHex:]
...
+ (UIColor *)RDLOT_colorWithHexString:(NSString *)stringToConvert {
  NSString *strippedString = [stringToConvert stringByReplacingOccurrencesOfString:@"#" withString:@""];
	...
	return [UIColor RDLOT_colorWithRGBHex:hexNum];
}
...
+ (UIColor *)searchForColorByName:(NSString *)cssColorName {
	...
	if (found) {
		...
		if (sscanf(after, "%x", &hex) == 1) {
			result = [self RDLOT_colorWithRGBHex:hex];
		}
	}
