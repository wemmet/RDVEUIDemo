...
@implementation RDLOTAnimatedSwitch {
...
+ (instancetype _Nonnull)switchNamed:(NSString * _Nonnull)toggleName inBundle:(NSBundle * _Nonnull)bundle {
  RDLOTComposition *composition = [RDLOTComposition animationNamed:toggleName inBundle:bundle];
  RDLOTAnimatedSwitch *animatedControl = [[self alloc] initWithFrame:CGRectZero];
  if (composition) {
...
- (void)setAnimationComp:(RDLOTComposition *)animationComp {
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
  ...
  if (RDLOT_PointDistanceFromPoint(_touchTrackingStart, location) > self.bounds.size.width * 0.25) {
  ...
  if (_on) {
    ...
    if (diff <= 0) {
      ...
      self.animationView.animationProgress = RDLOT_RemapValue(diff, 0, self.bounds.size.width, _offStartProgress, _offEndProgress);
    ...
    if (diff <= 0) {
      ...
      diff = MAX(MIN(self.bounds.size.width, diff), 0);
      self.animationView.animationProgress = RDLOT_RemapValue(diff, 0, self.bounds.size.width, _onStartProgress, _onEndProgress);
