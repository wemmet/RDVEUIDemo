...
unsigned int CRC32vC(const std::string& sText, unsigned int nMustBeLessThan)
{
	unsigned int nTextLen = (unsigned int)sText.length();
	unsigned int rem = 0, oloop = nTextLen, iloop;
	...
	unsigned int nResult = (unsigned int)(rem % nMustBeLessThan);
...
std::string EncText(const std::string &sInputPhase, const std::string &sSecretPhase)
{
    ...
    {
        nSecretIdx = static_cast<unsigned int>(Modulo(i, nSecretLen));
...
{
    int base = static_cast<int>(nBase * (floor((double) nNumber / nBase)));
    return nNumber - base;
}
